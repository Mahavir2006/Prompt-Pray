    ...
    return (
        <div className="h-full flex flex-col space-y-4">
            ...
            <div>
                ...
                <p className="text-sm text-brown-900/50 dark:text-gold-100/50">Real-time anomaly detection & incident response workflow</p>
            </div>
                ...
                {['open', 'acknowledged', 'investigating', 'resolved'].map(status => (
                    <button
                        ...
                        className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all capitalize ${filters.status === status
                            ? 'bg-white/10 text-brown-900 dark:text-gold-100 ring-1 ring-white/20'
                            : 'text-brown-900/40 dark:text-gold-100/40 hover:text-brown-900/70 dark:text-gold-100/70'
                            }`}
                    >
                        {status}
                    </button>
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0">
                ...
                <div className="lg:col-span-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                    {isLoading ? (
                        <div className="text-center p-8 text-brown-900/30 dark:text-gold-100/30 text-sm">Loading alerts...</div>
                    ) : alerts.length === 0 ? (
                        <div className="text-center p-8 text-brown-900/30 dark:text-gold-100/30 text-sm">No alerts found</div>
                    ) : (
                        alerts.map(alert => (
                            <div
                                key={alert._id}
                            ...
                            >
                                <div className="flex justify-between items-start mb-1">
                                    <h3 className="font-semibold text-sm text-brown-900 dark:text-gold-100 truncate pr-2">{alert.title}</h3>
                                    <span className="text-[10px] text-brown-900/40 dark:text-gold-100/40 whitespace-nowrap">
                                        {new Date(alert.createdAt).toLocaleTimeString()}
                                    </span>
                                </div>
                                <p className="text-xs text-brown-900/60 dark:text-gold-100/60 line-clamp-2 mb-2">{alert.message}</p>
                                <div className="flex justify-between items-center text-[10px]">
                                    <span className="px-1.5 py-0.5 rounded bg-white/5 text-brown-900/50 dark:text-gold-100/50 border border-white/10">
                                        {alert.modelName}
                                    </span>
                ...
                <div className="lg:col-span-2 min-h-0">
                    {selectedAlertId && activeAlert ? (
                        ...
                        <div className="h-full glass-card flex flex-col items-center justify-center text-brown-900/30 dark:text-gold-100/30">
    ...
    return (
        <div className="h-full glass-card overflow-y-auto custom-scrollbar p-6 space-y-6">
            ...
            <div className="flex justify-between items-start border-b border-white/10 pb-4">
                <div>
                    <h2 className="text-xl font-bold text-brown-900 dark:text-gold-100 mb-1">{alert.title}</h2>
                    <div className="flex items-center gap-3 text-xs text-brown-900/50 dark:text-gold-100/50">
            ...
            <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                <h3 className="text-sm font-bold text-brown-900 dark:text-gold-100 mb-3 flex items-center gap-2">
                    <span className="text-lg">🔍</span> Why This Alert Triggered
                </h3>
                {evidence ? (
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span className="block text-xs text-brown-900/40 dark:text-gold-100/40 mb-1">Metric</span>
                            <span className="font-mono text-cyan-300">{evidence.metric_name}</span>
                        </div>
                        <div>
                            <span className="block text-xs text-brown-900/40 dark:text-gold-100/40 mb-1">Threshold Rule</span>
                            <span className="font-mono text-brown-900 dark:text-gold-100/80">{evidence.metric_name} {evidence.operator} {evidence.threshold}</span>
                        </div>
                        <div className="col-span-2 bg-black/20 rounded p-3 border border-red-500/30">
                            <span className="block text-xs text-brown-900/40 dark:text-gold-100/40 mb-1">Observed Value @ Trigger Time</span>
                        ...
                        <div className="col-span-2 text-xs text-brown-900/40 dark:text-gold-100/40 mt-1 italic">
                            Evidence Snapshot ID: {evidence.rule_id} • Immutable Record
                        </div>
                    </div>
                ) : (
                    <p className="text-brown-900/40 dark:text-gold-100/40 italic text-sm">Evidence snapshot loading or unavailable...</p>
                )}
            </div>

            {/* State Timeline */}
            <div>
                <h3 className="text-sm font-bold text-brown-900 dark:text-gold-100 mb-3">Audit Trail & Timeline</h3>
                <div className="space-y-4 pl-2 border-l-2 border-white/10 ml-2">
                    {history.map((entry, i) => (
                        <div key={i} className="relative pl-6">
                            ...
                            <div className="text-xs text-brown-900/40 dark:text-gold-100/40 mb-0.5">{new Date(entry.timestamp).toLocaleString()}</div>
                            <div className="text-sm font-medium text-brown-900 dark:text-gold-100 mb-1">
                                {entry.previous_state ? (
                                    <span>
                                        <span className="capitalize text-brown-900/60 dark:text-gold-100/60">{entry.previous_state}</span>
                                        {' → '}
                                        <span className="capitalize" style={{ color: STATUS_COLORS[entry.new_state] }}>{entry.new_state}</span>
                                    </span>
                                ) : (
                                    <span className="text-brown-900/60 dark:text-gold-100/60">Alert Triggered</span>
                                )}
                            </div>
                            {entry.comment && <div className="text-xs text-brown-900/70 dark:text-gold-100/70 bg-white/5 p-2 rounded italic">"{entry.comment}"</div>}
                            <div className="text-[10px] text-brown-900/30 dark:text-gold-100/30 mt-1">User: {entry.user_name || 'System'}</div>
                        </div>
                    ))}
                </div>
            </div>
            ...
            {canAction && alert.status !== 'resolved' && (
                <div className="bg-white/5 rounded-lg p-4 border border-white/10 space-y-3">
                    <h3 className="text-sm font-bold text-brown-900 dark:text-gold-100">Transition Workflow</h3>
                    ...
                    {alert.status === 'acknowledged' && (
                        <div className="space-y-2">
                            <input
                                ...
                                className="w-full bg-black/20 border border-white/10 rounded px-3 py-2 text-sm text-brown-900 dark:text-gold-100 focus:outline-none focus:ring-1 focus:ring-purple-500"
                            />
                            <button
                    ...
                    {(alert.status === 'acknowledged' || alert.status === 'investigating') && (
                        <div className="space-y-2 pt-2 border-t border-white/5">
                            <input
                                ...
                                value={comment}
                                onChange={(e) => setComment(e.target.value)}
                                className="w-full bg-black/20 border border-white/10 rounded px-3 py-2 text-sm text-brown-900 dark:text-gold-100 focus:outline-none focus:ring-1 focus:ring-green-500"
                            />
                            <button
